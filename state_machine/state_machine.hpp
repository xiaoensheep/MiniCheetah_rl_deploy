...
namespace {
inline std::string ResolveProjectPath(const std::string& relative_path) {
    const std::filesystem::path cwd = GetAbsPath();
    const std::filesystem::path from_project_root = cwd / relative_path;
    const std::filesystem::path from_build_dir = cwd / ".." / relative_path;
